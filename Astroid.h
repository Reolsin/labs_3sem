...
namespace Prog2 {
	...
	public:
		...
		inline Astroid& setC(const Point& P) { C = P; return *this; };
		Astroid& setR(double r);

		inline Point getC() const { return C; };
		inline double getR() const { return R; };

		inline double length() const { return 6 * R; } //длина всей кривой
		double length(double t) const; //длину от 0 до t
		inline double curvature_radius(double t) const { return (1.5 * R * abs(sin(2 * t))); };	//радиус кривизны от t
		inline double area() const { return (value * R * R); };	//площадь
